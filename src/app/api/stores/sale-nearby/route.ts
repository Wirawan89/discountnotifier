    ...
    return NextResponse.json({
      message:
        scheduledStores.length > 0
          ? `Found ${scheduledStores.length} stores with ${totalDiscounts} current offers and ${totalPromotions} merchant promotions within ${MAX_NEARBY_WALKING_KM} km estimated walking distance of ${location || "your location"}`
